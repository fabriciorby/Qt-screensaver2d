...
void OpenGLWidget::initializeGL()
{
    ...
    for(int i=0; i<NUM_STARS; ++i)
    {
        ...
        float x = (qrand() / (float)RAND_MAX) * 5.0f - 2.5f;
        float y = ((qrand() / (float)RAND_MAX) * 5.0f) - 2.5f;
...
void OpenGLWidget::paintGL()
{
    ...
    for(int i=0; i<NUM_STARS; ++i)
    {
        glUniform4f(locTranslation, starPos[i].x(), starPos[i].y(), starPos[i].z(), 0);
...
{   
    ...
    for(int i=0; i<NUM_STARS; ++i)
    {
        ...
        pos.setX(pos.x() + elapsedTime / 25);
        pos.setY(pos.y() + elapsedTime / 5);

        if (pos.y() >= 2.5f)
        {
            float x = ((qrand() / (float)RAND_MAX) * 5.0f) - 2.5f;
            float y = ((qrand() / (float)RAND_MAX) * 2.5f) - 4.5f;
